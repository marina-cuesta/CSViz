...
## KDN at instance, class and data set level. 
## PARAMETERS: 
# - dataX is the data set with the feature variables
# - dataY is the vector with the target variable. It must be a factor
# - k is the percentage of data that is going to be considered as neighbors, 0<k<=1.
# - scale_data = {T,F} indicates if the data must be scaled to compute kdn
...
## Function to detect if a series has a an increasing tendency and, if so, the ix where it starts
## PARAMETERS:
# - series is the data series to examine
...
## PARAMETERS:
# - dataX_2D is the 2D data set with the features variables
# - dataY is the data set with the label variable. It must be a factor
# - k is the percentage of data to be considered in the kdn computation
...
## PARAMETERS:
# - dataX is the data set with the features variables
# - dataY is the data set with the label variable. It must be a factor
# - k is the percentage of data to be considered in the kdn computation, 0 < k <= 1
...
CSViz_plots_info <- function(results_CSViz_subspaces, min_data, max_kdn){
...
# #############################################################################
# 
# ## Function to obtain a table with the detailed information about the the obtaines subspaces 
# ## PARAMETERS:
# # - results_CSViz_subspaces is the return of the CSViz_subspaces_computation function
# # - min_data: minimum allowed percentage of data in a subspace for it to be visualized
# # - max_kdn: maximum kDN allowed for a subspace to be displayed
# 
# CSViz_table_info <- function(results_CSViz_subspaces, min_data,max_kdn){
#   
#   #######################################
#   #### Checking initial requirements ####
#   #######################################
...
################################################################
################################################################
######   VISUALIZATION OF SUBSPACES: STORYTELLING PLOT    ######
################################################################
################################################################

## Function to visualize the obtained subspaces through a storytelling of
# what data is available and its corresponding subspace in each iteration
## PARAMETERS:
# - results_CSViz_subspaces is the return of the CSViz_subspaces_computation function
# - min_data: minimum allowed percentage of data in a subspace for it to be visualized
# - max_kdn: maximum kDN allowed for a subspace to be displayed

CSViz_storytelling <- function(results_CSViz_subspaces,min_data,max_data){
  ...
  #############################################
  ##### Obtaining plots for storytelling  ##### 
  #############################################
  
  ## auxliar variables to compute information
  sum_data=0
  sum_data_classes=numeric(n_classes)
  ...
  ## list to store all the subspaces
  list_grid=list()
  ...
  for (subspace in 1:(n_subspaces)){
    ...
    ## KDN value of the subspace in this iteration
    kdn_dataset_level=round(obtained_subspaces[[subspace]]$kdn,2)
    ...
    #################################################
    ########  PLOT OF THE AVAILABLE DATA     ########
    #################################################
    
    ## Data for plotting
    dataX_plot=dataX[c(ix_subspace_iter,ix_not_subspace_iter),variables_iter]
    dataY_plot=dataY[c(ix_subspace_iter,ix_not_subspace_iter)]
    ...
    ## initialize the plot for the corresponding data
    ...
    ## scatter plot if both variables are numeric (more than two unique variables)
    if(all(count_unique_values>2)){
      plot_data = plot_data + ggplot2::geom_point(size = 2.2)+
        # setting x and y axes limits
        ggplot2::coord_cartesian(xlim=c(x_lim_inf,x_lim_sup), ylim=c(y_lim_inf,y_lim_sup))
    ...
    if(all(count_unique_values>2)){
      ...
      # plotting geom jitter
      set.seed(1234)
      plot_subspace = plot_subspace+ ggplot2::geom_jitter(aes(x=factor(dataX_plot[,1]),y=dataX_plot[,2],
    ...
    ###############################################
    ##### GRID PLOT AVAILABLE DATA + SUBSPACE ##### 
    ###############################################
  ...
  ## grid of plots and tables
  plot_storytelling=grid.arrange(arrangeGrob(grobs =list_grid, nrow=length(list_grid)), legend, nrow = 2, heights = c(10, 0.4))
  
  return(plot_storytelling)
}
